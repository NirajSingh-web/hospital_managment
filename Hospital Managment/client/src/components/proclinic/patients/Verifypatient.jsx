...
const Verifypatient = () => {
  ...
  // when apihit value is true then it will change value false for dependency for next api call
  apihit &&
    setTimeout(() => {
      setApihit(false);
    }, 1000);
  ...
  // get submit date from pendingdata and store in SubmitDate array and apply same date for take value today
  useEffect(() => {
    pendingdata &&
  ...
  const handleonchange = (e) => {
    const filtereddata = pendingdata.filter((i) =>
      i["createdAt"]
        ...
        .join("-")
        .includes(
          e.target.value === "Today Patient List"
            ? currentDate()
            : e.target.value
        )
    );
    ...
    const searchfiltereddata = (
      ...
      (e) =>
        ...
        e["email"]
          ...
          .toLowerCase()
          .includes(s.target.value.trim().toLowerCase()) ||
        e["phone"].toString().includes(s.target.value.trim()) ||
        e["dob"]
          .trim()
          .toLowerCase()
          .includes(s.target.value.trim().toLowerCase())
  ...
  const Handleonapproval = async (e) => {
    ...
    if (confirmation) {
      ...
      if (data === "Approved") {
        ...
        setalert({ AlertType: "warning", AlertData: data });
        setloader(false);
      }
    }
  };
              ...
              <div className=" sticky top-0 z-10 ">
                <div className="p-3 flex justify-between   z-20 text-white bg-[hsl(210,56%,25%)]">
                  <div className="flex items-center gap-2">
                    ...
                    >
                      <option value="">All Patient Data</option>
                      {SubmitDatelist &&
              ...
              >
                {(searchfiltereddata
                  ...
                  searchfiltereddata || Datefiltereddata || pendingdata ? (
                    <>
                      <div
                        className={`card  w-[60%] max-sm:w-full  bg-[hsl(210,56%,25%)] text-white ${
                          index > 0 && "mt-4"
                        }`}
                        key={index}
                      >
